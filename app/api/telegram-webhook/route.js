...
export async function POST(request) {
  try {
    ...
    // il bot è appena stato aggiunto (o rimosso, o promosso) in un gruppo
    ...
    if (membership) {
      ...
      const appenaEntrato = ['member', 'administrator'].includes(statoNuovo) &&
                             ['left', 'kicked'].includes(statoVecchio);
      const appenaPromosso = statoNuovo === 'administrator' && statoVecchio === 'member';
      ...
      if (appenaEntrato) {
      ...
      } else if (appenaPromosso) {
        // era già registrato come membro semplice: ora che è admin, prova
        // a generare il link e completa la registrazione
        const link = await generaLinkInvito(chatId);
        ...
        await inviaMessaggioTelegram(
          chatId,
          link
            ? `Ora sono amministratore: link d'invito generato. Questo gruppo è pronto per Maisola.`
            : `Sono admin, ma non riesco ancora a generare il link — controlla di avermi dato il permesso "Invita utenti tramite link".`
        );
      }
